...
コマンドは**構造化argvまたはshell AST検証で全体を検証**し、allowlist照合はパース後の`argv[0]`に対して行う。

> **メタ文字のdenylistで実装しないこと。** 改行・復帰・単独`&`が残り、`npm test`＋改行＋任意コマンドでallowlistとwrite scopeを迂回できる。

規則の正本は`.claude/rules/permissions.md` §4による。

## 4.1 環境変数のsanitization

§1.2は「秘密情報が必要な処理は環境変数またはsecret managerを経由する」と定めるが、**これはAgentRunの環境に秘密情報を置いてよいという意味ではない。**

Agentが起動するBashとその子プロセス（テストコード、ビルドスクリプト、プラグイン、依存）は、**プロセス環境をそのまま継承する。** 環境変数へ秘密情報があれば、§1.1のパスdenyも§2のredactionも迂回して到達できる。read denyが守るのはファイルであって、プロセス環境ではない。

強制側は**全AgentRunに対して**次を課す。

- Agentプロセスの環境を**allowlist方式で構築する。** 親環境をそのまま継承せず、実行に必要な変数（`PATH`、`HOME`、言語ランタイムの必須変数等）だけを明示的に渡す。
- 秘密情報を保持する変数（APIキー、トークン、DB資格情報、CI secret）を**除去する。**
- 隔離環境を**secret非搭載**とする（§3）。これは§3の「Network遮断・secret非搭載の隔離環境」の`secret非搭載`の実装内容である。
- 秘密情報を必要とする処理は、**Agentの権限では実行しない。** 信頼済みRunnerが自らの権限で実行し、redaction済みの結果を証跡としてAgentへ渡す（§2.1と同じ構造）。

> **§1.2の「環境変数を経由する」は、アプリケーション実行時の設計方針である。** 未信頼コードを評価するAgentRunの環境設計とは別の話であり、後者では秘密情報を環境から除去する。
