...
Runtime Contextの起動は**原則として行わない**（設計書 §6.2）。「原則」であり、例外なしの禁止ではない。

- 実DBの起動、本番相当の全体構成の起動が必要になった場合は、**UT/ITの切り分けが誤っている。** 差し戻す。それはIntegration Testの領分である（§4）。
- 軽量なcontext（対象クラスの協調に必要な最小限の構成）で足りる場合は、**§2の「実行速度」と「対象」を満たす限り許容する。** 頻繁に全関連UTを実行できる速度を維持でき、対象が一つのクラス・関数・小さな協調単位に収まっていることが条件である。
- 判断が分かれる場合は、Test Reviewerの評価対象として理由をテスト計画へ記録する。**速度が劣化して全UTを頻繁に実行できなくなった時点で、それはUTとして成立していない。**
...
### 8.1 UI検証時点で`CODE_REVIEW_TARGET`は存在しない

上記の「固定されたreview target」を`CODE_REVIEW_TARGET`と解してはならない。PHASE-8の順序は`INTEGRATION_TEST` → `UI_VERIFICATION` → `CODE_REVIEW_TARGET`であり、**`CODE_REVIEW_TARGET`はUI検証より後に固定される**（設計書 §7.2）。これを待つと、UI変更タスクが相互待ちでfail-closed停止する。

UI検証時点の固定対象は次のとおり解決する（設計書 §3.8「PHASE-8途中のレビュー対象」。詳細は`.claude/rules/worktree-isolation.md` §5）。

| 用途 | 参照先 |
|---|---|
| `ui_change`の再検証、評価済みproduction codeの基準点 | PHASE-7の`kind: implementation_review` targetの`commit_sha` |
| previewをビルドするcommit、証跡を束縛するSHA | Integration Test Engineerのagent-runの`result_commit`（PHASE-7 `commit_sha`の**子孫であること**を検証する） |
| 変更ファイル一覧 | `changed_files_manifest`（`docs/status/changes/<task>.yaml`） |

**この解決結果をGateRunとagent-runへ記録する。** 解決できない場合はゲートを開始せずfail-closedとする。「対象SHA不一致」の判定は、上記で解決したSHAと証跡のSHAの一致を指す。
