...
worktreeは**実行時作業領域**としても使える（設計書 §3.6.3）。使い捨てworktreeでビルド・テストを実行する場合、**run終了時に破棄し、次のrunへ状態を持ち越さない。** 持ち越すとテスト結果が前のrunに依存し、証跡がcommitへ束縛されなくなる。
...
- Evaluatorは`commit_sha`からコードを読み、targetファイルは**現在のcheckoutから**読む。
