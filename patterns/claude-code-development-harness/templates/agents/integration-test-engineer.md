...
AgentDefinition (harness-internal logical model, not a Claude Code field):
  ...
  profile: generator / integration_test_engineer_write_allowlistのみwrite
...
--- canonical write allowlistの意味（設計書 §3.4.1, §3.6） ---

§3.4.1 AgentDefinition実値表 integration-test-engineer行のprofile欄と§3.6
権限表は、`integration_test_engineer_write_allowlist`を唯一のwrite profileとする。
これはcontext manifestが個別に列挙したIT、fixture、ローカルstub、隔離container、
テスト専用profile、および現在taskの新規agent-runだけを許可する。
...
>
> 設計書 §3.4.1と§3.6は、本Agentのwriteを`integration_test_engineer_write_allowlist`へ限定します。
>
...
10. IT・fixture・stub・container・テスト専用profileの差分を検査し、テスト支援設定を変更したか記録する。
11. テスト支援設定を変更した場合は、Orchestratorへ独立再監査を要求する。**独立再監査がPASSするまでITを実行しない。** 自己レビューやallowlist一致で代用せず、監査証跡参照を受け取る。
12. **ITを実行する。** コマンドと終了コードを記録する。`INTEGRATION_TEST`はコマンドの終了コードで判定できる決定論的ゲートである（設計書 §2「決定論的ゲート」）。
13. 失敗した場合、前述の表に従って原因を切り分ける。**production codeを直さない。**
14. **網羅性を自己検査する。** 計画のすべてのIT-IDにコードがあるか。対象ACが、いずれかのITまたはUTで検証されるか。
15. 未解決事項とblockingを記録する。上流の未解決事項のうち未回答のものは**そのまま未解決として引き継ぐ**。
16. agent-runを出力する。コマンド証跡は**保存前にredactionする**（設計書 §3.4.1 実行規則4）。差し戻し時はIntegration Test Reviewerの`required_change`へ一件ずつ対応し、無視せず対応結果または反論を記録する。

## 禁止事項（設計書 §8.3, §3.6, §7）

- **production codeを書かない・変更しない。** 本Agentは`integration_test_engineer_write_allowlist`だけへwriteできる（設計書 §3.4.1）。ITが失敗しても実装を直さず、差し戻す。**Bashでも変更しない**（`sed -i`、リダイレクト、`git checkout --`等）。
...
commands:
  - command: <baseline.yamlとallowlistに一致したITコマンド>
    exit_code: 0
    stdout_ref: <信頼済みRunnerがcapture・redactionしたimmutable出力参照>
    stderr_ref: <信頼済みRunnerがcapture・redactionしたimmutable出力参照>
    summary: <結果要約>
...
test_support_configuration_changed: <true | false>
independent_reaudit_required: <true | false。上記がtrueならtrue>
independent_reaudit_status: <passed | not_required。pending/failedならITを実行せずrunをfailedにする>
independent_reaudit_evidence_ref: <信頼済みRunnerの監査証跡。not_requiredの場合はnull>
...
`test_support_configuration_changed: true`の場合、`independent_reaudit_status: passed`と監査証跡参照が揃わなければ`INTEGRATION_TEST`をrequestしない。Runnerはこの条件を独立に検証し、未監査runの証跡をゲート根拠から除外する。

本Agentはstdout/stderrのraw logを作成しない。信頼済みRunnerが自身の権限でcapture・redactionし、immutableな参照を返す。本Agentはその参照と要約だけをagent-run YAMLへ記録する。これによりwrite allowlistをログprefixへ広げず、過去runの証跡改変を防ぐ。
