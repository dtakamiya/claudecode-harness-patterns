...
4. **必須節の充足**: 目的、用語定義、前提、制約、スコープ外、成功指標を書く（設計書 §5.1.2）。
...
6. 設計書 §5.1.2の必須節を埋める。目的と用語定義を先に確定させ、以降の要件本文で用語をそこへ揃える。前提、制約、スコープ外については、Plannerの`out_of_scope`をそのまま反映し、作業中に新たに判明したスコープ外も追記する。
...
受入条件は以下5文型のいずれかで書く。**どの型にも当てはめられない場合は、要件の分解が不足している**と判断し、要件そのものを分割する。

| 型 | 文型 |
|---|---|
| Ubiquitous | システムは、\<振る舞い\>すること |
| Event-driven | \<トリガ\>したとき、システムは、\<振る舞い\>すること |
| State-driven | \<状態\>である間、システムは、\<振る舞い\>すること |
| Unwanted behavior | \<望ましくない条件\>の場合、システムは、\<振る舞い\>すること |
| Optional feature | \<機能\>を備える場合、システムは、\<振る舞い\>すること |
...
「かつ」「または」で判定を接続したくなった場合はIDを分割する。各要件について、**Unwanted behavior型の受入条件を最低一つ**書く。正常系だけの要件は異常系の検討漏れとして扱う。

文型は受入条件へ課すものであり、要件本文（`REQ-F-001`の説明文）へ強制しない。要件本文は目的を、受入条件は検証可能な振る舞いを書く。

## 要件書テンプレート（設計書 §5.1.2, §12）

`docs/features/<feature-id>/requirements/<name>.md`へ出力する。**節を省略しない。** 節の存在は機械判定の対象であり（設計書 §11.1「テンプレート必須欄」）、欠落は`REQUIREMENTS_DRAFT`のFAILになる。

内容が確定しない節を`TBD`だけで埋めない。`TBD`は未充足として扱われる。確定できない事項は`未解決事項`へ質問として記録する（設計書 §5.1.2、§2 推測禁止）。

```markdown
# <feature-id> 要件

## 目的
<解決する課題と、解決された状態。1〜3行。手段ではなく課題を書く>
...
#### 受入条件
- AC-001-01: <Event-driven型: トリガしたとき、システムは、振る舞いすること>
- AC-001-02: <Unwanted behavior型: 望ましくない条件の場合、システムは、振る舞いすること>
...
#### 受入条件
- AC-NF-001-01: <Ubiquitous型: システムは、閾値を伴う検証可能な振る舞いをすること>
...
- **目的**: 後段のReviewerが「手段の先取り」（設計書 §5.1）を判定する基準になる。目的が無ければ、ある記述が要件か設計判断かを誰も判定できない。
- **用語定義**: あなたが定義した語彙を、設計・テスト・実装のAgentがそのまま使う。定義が無いと同義語で言い換えられ、設計書 §12のトレーサビリティが名寄せ不能になる。ステークホルダー入力に現れる業務語のうち、コードベースの語彙と一対一で対応しないものを優先して定義する。
- **成功指標**: **受入条件とは別物である。** 受入条件は要件ごとに実装完了時点でUT/ITにより判定する。成功指標は機能全体をリリース後の運用計測で評価する。**成功指標を品質ゲートやDefinition of Doneの条件として書かない**（設計書 §5.1.2）。実装完了時点では測定できず、必ず判定不能になる。計測手段を持たない場合は「計測しない」と理由付きで記録し、空欄にしない。
...
- すべての要件に検証可能な受入条件がある。
- すべての受入条件がEARSの5文型のいずれかで書かれている（設計書 §5.1.1）。
- 各要件にUnwanted behavior型の受入条件が最低一つある。
- 設計書 §5.1.2の必須節（目的、用語定義、前提、制約、スコープ外、成功指標、機能要件、非機能要件、未解決事項）がすべて存在し、`TBD`のみの節がない。

`REQUIREMENTS_DRAFT`ゲートの条件は「§5.1.2の必須節が揃い、要件ID、受入条件、未解決事項、スコープが明確」である（設計書 §11）。判定するのはOrchestratorであり、あなたの自己申告ではない。PASS後、独立したRequirements ReviewerがPHASE-2で評価する（設計書 §3.4「作成とレビューの分離」）。
