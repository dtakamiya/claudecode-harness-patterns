...
--- Bash allowlist（両モードで必須） ---

設計書 §3.6 TDD Generator行の強制手段は
「permissions＋sandbox＋**Bash allowlist**＋HookまたはRunner検証」であり、
Shell範囲を「build/test限定、Network原則なし」と定める。
Bash allowlistはCompatibleモードだけの代替手段ではなく、**Fullモードでも必須**とする。

本文は実行すべきコマンドの出所を`baseline.yaml`（設計書 §5.0の実測結果）と
するが、**`baseline.yaml`はGit内の編集可能なファイルであり、それ自体は
信頼境界ではない**。baselineが改ざんされていれば、本文の指示に従うだけで
...
access_policy（論理モデル。宣言だけでは書込み境界にならない）:
  ...
  writable_phase_7:
    ...
    - docs/features/**/reviews/targets/<task>-implementation.yaml
      # 設計書 §3.8「Generatorがチェックポイントコミットを作成し」および
      # §6.6のIMPLEMENTATION_REVIEW_TARGET固定は、Generatorの作業である。
      # §3.6 TDD Generator行の論理Write範囲
      # 「対象モジュール、テスト、agent-run成果物」はreview targetを
      # 明記しないが、§3.8がPHASE-7でのtarget固定を要求する以上、
      # これは記述の欠落である。reviews/**全体ではなく、自タスクの
      # targetファイル一点へ限定して解決する。
