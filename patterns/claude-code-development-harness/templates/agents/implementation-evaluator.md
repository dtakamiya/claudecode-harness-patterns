...
設計書 §10.1は、review targetを伴う評価を次の三境界へ分離する。

- `evaluated_code_commit`: `review_target.commit_sha`。実際にコードを読むcommit。
- `evaluated_commit`: review targetと`changes/<task>.yaml`を含む、開始前の
  `PhaseRun.evaluation_input_commit`。
- `evaluation_step_input_commit`: 本Agentのstep開始時点。先頭Evaluatorでは
  `evaluated_commit`、後続Evaluatorでは直前stepのoutput。
- `evaluation_output_commit`: review結果と本Agentのagent-runだけを追加した後、
  信頼済みRunnerが作成しGateRunと`PhaseRun.result_commit`へ記録するcommit。

本Agentが作成するreview結果とagent-runには`evaluation_output_commit`以外の三つを記録する。
`evaluation_output_commit`は、自身を含むcommit SHAを自己参照できないため記録しない。
信頼済みRunnerが出力commitを作成後、control planeへ記録する。

確認項目Aでは、`evaluated_code_commit`から`evaluated_commit`までの差分をreview
targetと`changes/<task>.yaml`の正確な2パスへ限定する。Runnerはさらに、
`evaluation_step_input_commit`から`evaluation_output_commit`までを本Agentの新規review結果と
新規agent-runの正確な2パスへ限定する。production code、UT、既存証跡、他taskの
成果物がいずれかの差分へ含まれる場合はblockingとする。
...
phase: PHASE-7
evaluated_commit: <PhaseRunのevaluation_input_commitと一致させる（設計書 §10.1）>
...
あなたはEvaluatorであり、固定されたレビュー対象をreadし、テストを実行するだけで新たなcommitを作らない。`evaluated_commit`はPhaseRunの`evaluation_input_commit`と一致させる。出力commitは信頼済みRunnerが作成し、本Agentの成果物へ自己記載しない（設計書 §10.1、templates/agents/development-orchestrator.md「run種別」表）。
