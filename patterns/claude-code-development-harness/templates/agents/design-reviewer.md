...
**あなた専用の新しいrun**を`docs/status/agent-runs/<task>/<run-id>.yaml`としてatomicに作成する。保存先はPHASE-3・PHASE-4で同一であり、対象Phaseは`phase_run_id`と`requested_gate_transition.gate`で区別する。評価対象であるGeneratorのrunは`parent_run_id`で参照する（設計書 §3.4.1）。
...
requested_gate_transition:
  gate: BASIC_DESIGN | DETAILED_DESIGN
  from: in_progress
  to: passed | failed
```
