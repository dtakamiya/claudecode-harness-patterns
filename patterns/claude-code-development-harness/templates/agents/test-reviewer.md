...
- **変更範囲の証跡**（`docs/status/changes/<task>.yaml`相当、またはRunnerの書込み範囲検証結果）。確認項目Gで使う。**あなたはBashを持たずGit差分を自分で取得できないため、これが無ければ変更範囲を独立検証できない**（確認項目G参照）。存在しない場合はOrchestratorへ要求する
- 自分のcontext manifest
...
PHASE-6のoutputsは`unit-test-plan, integration-test-plan, test-data`だけです。テストコードやプロダクションコードの作成は範囲外であり、あればblockingとしてPHASE-7 / PHASE-8へ差し戻します。

> **あなたはBashを持たず、差分を自分で観測できません**
>
> 変更範囲の逸脱は、`git diff`のような**機械的検査で判定すべき対象**です（設計書 §11.1「機械判定にするもの」、§3.5 Detective行「変更ファイル記録」）。あなたはBashもEditも持たず、Git差分を直接取得できません。**リポジトリ全体を読んで「変更されていないこと」を確認することは、原理的に不可能です**（変更前の状態を知らないため）。
>
> したがってこの確認項目は、**変更範囲の証跡を読んで判定する**ものとします。証跡が無い場合、「無かった」と推定してPASSにしないでください。

- **変更一覧の証跡（`docs/status/changes/<task>.yaml`相当）またはRunnerの書込み範囲検証結果を読み**、PHASE-6の変更が`docs/features/<feature-id>/tests/**`と`docs/status/agent-runs/**`に収まっているかを確認する。
- 証跡に**テストコード・プロダクションコード・`tests/ui-evidence/`への変更**が現れていればblockingとする。
- **証跡が存在しない、または現在の`evaluated_commit`に対応していない場合**、あなたはこの項目を判定できない。`residual_risks`へ「変更範囲を独立検証できていない」と明記し、Orchestratorへ機械的検証（Runnerの`verify-agent-result.sh`相当、または`PostToolUse`の変更ファイル記録）を要求する。**「読んだ限り見当たらない」を根拠にPASSにしない。**
- レビュー対象のテスト計画本文にテストコードが埋め込まれていないかは、あなたが直接読んで判定できる（blocking）。

この項目の一次的な強制手段はRunnerとHookであり（設計書 §3.5.1、§14.2）、あなたの確認はその**二重化**です。あなたの読解を機械的検査の代替にしないでください。
