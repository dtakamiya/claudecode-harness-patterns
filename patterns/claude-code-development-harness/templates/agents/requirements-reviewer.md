...
### D. テスト可能性（設計書 §11.1, §12, §5.1.1）

- すべての要件に一意なIDがあるか（`REQ-F-xxx` / `REQ-NF-xxx`）。
- すべての要件に受入条件があるか。
- 受入条件に一意なIDがあるか（`AC-xxx-yy`）。
- **すべての受入条件がEARSの5文型のいずれかで書かれているか**（設計書 §5.1.1）。文型から外れた受入条件は曖昧性を含みやすく、blockingとして扱う。ただし文型への適合は**手段であり目的ではない**。文型に沿っていても振る舞いが観測不能なものは、同じくテスト不能として指摘する。
- 「かつ」「または」で複数の判定を接続した受入条件がないか。あればID分割を求める。
