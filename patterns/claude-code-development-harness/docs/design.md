...
| コードレビュー | 不要 | 不要 | Code Reviewer / Adversarial Reviewer（金融処理時） | Evaluator専用 |
...
| `PHASE-9` コードレビュー | fixed-review-target, test-evidence | code-review, adversarial-review結果または理由付き除外判定 | `INTEGRATION_TEST` | `CODE_REVIEW` | code-reviewer, adversarial-reviewer |
...
| adversarial-reviewer | evaluator | PHASE-9 | — | evaluator / 作成者・Code Reviewerから独立 |
...
| Adversarial Reviewer | 金融処理の前提を崩し、二重処理・冪等性、競合・順序逆転、成否不明・部分障害、精度・丸め・通貨、日付境界、内部不正・監査、照合・復旧を検査 | 作成者や他Reviewerの判断を追認せず、独立した証跡で評価する |
