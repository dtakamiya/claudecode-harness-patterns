...
| integration-test-engineer | generator | PHASE-8 | tdd-development@1 | generator / `integration_test_engineer_write_allowlist`のみwrite |
...
`integration_test_engineer_write_allowlist`をIntegration Test Engineerの唯一のwrite profileとする。許可するのは、context manifestが個別のcanonical pathで列挙したIntegration Testコード、テストfixture、ローカルstub定義、隔離container定義、テスト専用profile、および`docs/status/agent-runs/<current-task>/<new-run-id>.yaml`の新規作成だけである。production code、PHASE-7のUnit Test、本番接続設定、ビルド設定、CI設定、依存定義、他taskまたは既存のagent-runは含めない。

stdout / stderrのraw logはIntegration Test Engineerへ書かせず、信頼済みRunnerがcapture・redactionしてimmutableな参照を返す。Agentは参照と要約だけを上記agent-run YAMLへ記録する。ログ用prefixをallowlistへ追加してはならない。

PHASE-8のcontext manifestはprofile名と個別pathを次のように併記し、Context Builder、permissions、Hook、sandbox、Runnerは同じprofileを参照して積集合を強制する。
...
- Integration Test Engineerのagent-runは`test_support_configuration_changed`、`independent_reaudit_required`、`independent_reaudit_status`、`independent_reaudit_evidence_ref`を記録する。設定変更時は独立再監査のstatusが`passed`で証跡参照が検証できるまでITを実行せず、`INTEGRATION_TEST`をrequestしない。
...
- PHASE-7の`kind: implementation_review` targetを読み、`commit_sha`を**評価済みproduction codeの基準点**として得る。Integration Test Engineerが`integration_test_engineer_write_allowlist`外を変更していないことの検証は、この基準点との差分で行う。
...
要件書は人間の合意記録であると同時に、**後続する全AgentRunの権威ある入力**である（§3.3 コンテキスト選定原則）。実装がAgentによって高速化されるほど、律速は「何を作るかを正確に記述すること」へ移り、要件書の欠落はそのまま誤った実装の量産として現れる。したがって本工程を省略の対象としない。

- 機能要件と非機能要件に一意なIDを付与する。例: REQ-F-001、REQ-NF-001

- 各要件に検証可能な受入条件を付与する。例: AC-001-01

- 受入条件はEARS（Easy Approach to Requirements Syntax）の文型で記述する。

- 目的、用語定義、前提、制約、スコープ外、成功指標、未解決事項を明示する。

- 設計・実装上の手段を早期に固定しすぎない。

### 5.1.1 受入条件の記述形式（EARS）

受入条件は自然文で書くと書き手ごとに粒度と曖昧性がぶれ、PHASE-6のTest Designerが正常・異常・境界へ写像できない。以下の5文型のいずれかで記述し、どの型にも当てはまらない場合は要件の分解が不足していると判断する。

| 型 | 文型 | 用途 |
|---|---|---|
| Ubiquitous | システムは、\<振る舞い\>すること | 常時成立する性質 |
| Event-driven | \<トリガ\>したとき、システムは、\<振る舞い\>すること | 事象への応答 |
| State-driven | \<状態\>である間、システムは、\<振る舞い\>すること | 状態依存の振る舞い |
| Unwanted behavior | \<望ましくない条件\>の場合、システムは、\<振る舞い\>すること | 異常系・エラー処理 |
| Optional feature | \<機能\>を備える場合、システムは、\<振る舞い\>すること | 構成により有無が変わる機能 |

記述例。

```markdown
- AC-001-01: 利用者が有効な認証情報を送信したとき、システムは、セッショントークンを発行すること
- AC-001-02: 認証に5回連続で失敗した場合、システムは、当該アカウントを15分間ロックすること
- AC-NF-001-01: システムは、認証APIの応答を95パーセンタイルで300ms以内に返すこと
```

一つの受入条件へ複数の判定を詰め込まない。「かつ」「または」で接続したくなった場合はIDを分割する。異常系はUnwanted behavior型で明示的に書き、正常系だけの受入条件をPASSさせない（§11.1）。

### 5.1.2 要件書の構成

要件書に含める節を次のとおり定める。`REQUIREMENTS_DRAFT`は、必須節の存在と各節の充足を条件とする（§11）。節の欠落は機械判定できるため、§11.1の機械判定側へ寄せる。

| 節 | 必須 | 内容 | 欠落したときに起きること |
|---|---|---|---|
| 目的 | 必須 | 解決する課題と、解決された状態。1〜3行 | 手段の先取り（§5.1）を検出する基準が無くなり、要件の妥当性を誰も判定できない |
| 用語定義 | 必須 | ドメイン語彙と、その定義 | 要件・設計・テストで語彙が揺れ、§12のトレーサビリティが名寄せ不能になる |
| 前提 | 必須 | 成立を仮定している事項 | 仮定が暗黙化し、崩れたときに影響範囲を特定できない |
| 制約 | 必須 | 守るべき制約と、その指定元 | 指定元のない技術選定がAnalystの推測として混入する（§8.3） |
| スコープ外 | 必須 | 対象外とする事項と理由 | 後続工程が範囲を自己拡大し、レビュー対象が固定できない（§3.8） |
| 成功指標 | 条件付き必須 | 機能全体の達成を測る指標、計測方法、計測時期 | 目的の達成を検証する手段が無くなる。計測手段を持たない場合は「計測しない」と理由付きで記録する |
| 機能要件 | 必須 | `REQ-F-xxx`と受入条件 | — |
| 非機能要件 | 必須 | `REQ-NF-xxx`と受入条件 | 性能・可用性・セキュリティが設計工程の暗黙判断になる |
| 未解決事項 | 必須 | 質問、blocking判定、確認先 | 推測での穴埋めが発生する（§2 推測禁止） |

**成功指標と受入条件を混同しない。** 両者は判定時点と判定単位が異なる。

| 観点 | 受入条件（`AC-xxx-yy`） | 成功指標 |
|---|---|---|
| 単位 | 要件ごと | 機能全体 |
| 判定時点 | 実装完了時点 | リリース後の計測時点 |
| 判定手段 | UT / IT（§12で追跡） | 運用計測 |
| ゲート適用 | `UNIT_TEST_GREEN`等で必須 | **品質ゲートおよび§15 Definition of Doneの判定に使わない** |

成功指標は実装完了時点では測定できないため、ゲート条件へ持ち込むと必ず判定不能になる。要件書へ記録し、PHASE-10の完了監査では「記録されていること」だけを確認する。

`REQUIREMENTS_DRAFT`の判定において、必須節が存在しても内容が`TBD`のみである場合は未充足として扱う（fail-closed、§3.4.1 実行規則3）。確定できない事項は空欄や`TBD`ではなく、未解決事項へ質問として記録する。
...
これらのパスと`progress.yaml`は、**コードcommit系列の外にあるappend-only control-state store**の論理パスとする。Gitで実装する場合はコードworktreeとは別のcontrol-state ref / worktreeへ保存し、そのHEADを`progress.yaml.current_commit`として扱わない。コードworktreeへread-onlyでmaterializeしても追跡対象へ含めず、control-state更新によってコードHEADを進めない。`current_commit`、PhaseRunのcommit field、次PhaseRunの`input_commit`はコード・成果物系列だけを指す。これにより、commit作成後にGateRun / PhaseRunへSHAを記録しても自身を含むcommit SHAの自己参照を生じさせない。
...
PhaseRunの`gate_run_refs`にはGateRun IDではなく、リポジトリルートからのcanonical pathを列挙する。Orchestratorは各参照について、パスが`docs/status/gate-runs/`配下に正規化されること、`..`によるtraversalを含まないこと、symlinkではないこと、ファイル名の`<gate-run-id>`が内部の`gate_run_id`と一致すること、内部の`phase_run_id`が参照元PhaseRunと一致することを検証する。GeneratorのAgentRunはPhaseRunの`input_commit`から開始し、評価入力checkpointを生成する。review targetを伴わないPhaseでは、成果物を評価するGateRun、Artifact、TestEvidence、ReviewTargetの`evaluated_commit`をPhaseRunの`result_commit`と一致させる。review targetを伴うPhaseでは、Evaluator開始前の`evaluation_input_commit`とEvaluator成果物追加後の`result_commit`を分離し、前者を各評価成果物の`evaluated_commit`、後者をEvaluator GateRunの`evaluation_output_commit`および次PhaseRunの`input_commit`と一致させる。

ただし、レビュー対象を固定するPhaseでは`evaluated_commit`と**実際に評価したコードのcommit**が一致しない。§3.8の`commit_sha`はレビュー対象のコードを固定したcheckpoint commitであり、review target成果物と`changes/<task>.yaml`はその後に作られるため、`evaluation_input_commit`は必ず`commit_sha`の子孫になる。**targetファイルは自身を含むcommitのSHAを自身へ記載できない**（記載した時点でSHAが変わる）ため、この差は構造上不可避である。

したがってreview targetを伴う評価では、コード、評価入力、評価出力を三つの境界へ分離する。

- `evaluated_code_commit`: 対応する`review_target.commit_sha`と一致させる。Evaluatorが実際にコードを読み、テストを実行したcommitを表す。
- `evaluated_commit`: review targetと`changes/<task>.yaml`を含む、Evaluator開始前の不変な入力checkpointとする。PhaseRunの`evaluation_input_commit`および参照元Generator AgentRunの`result_commit`と一致させる。
- `evaluation_step_input_commit`: 一人のEvaluatorが開始する時点のcheckpointとする。最初のEvaluatorでは`evaluated_commit`、後続Evaluatorでは直前stepの`evaluation_output_commit`と一致させる。
- `evaluation_output_commit`: 当該Evaluatorのreview結果とagent-runを追加した後、信頼済みRunnerが作る出力checkpointとする。Evaluator自身が書くreview結果またはagent-runへこのSHAを記載してはならない。Runnerがcommit作成後、コード系列外のcontrol-state GateRun / PhaseRunへ記録する。

Orchestratorと信頼済みRunnerは二段階の差分を別々に検証する。

- `evaluated_code_commit`から`evaluated_commit`までの許可差分は、review targetと`docs/status/changes/<task>.yaml`の正確な2パスだけとする。
- 各`evaluation_step_input_commit`から対応する`evaluation_output_commit`までの許可差分は、当該Evaluatorが新規作成するreview結果とagent-runの正確な2パスだけとする。
- 各パスはcurrent task / runへ解決し、create-only、canonical path、非symlinkを検証する。どちらかの差分にproduction codeまたはテストコード、他taskの成果物、既存証跡の変更が含まれる場合はfail-closedで拒否する。

review結果とevaluator profileのagent-runは`evaluated_code_commit`、`evaluated_commit`、`evaluation_step_input_commit`を併記し、`evaluation_output_commit`を自己申告しない。PhaseRunはEvaluatorごとに一つのstepを順序付きで持ち、PHASE-9ではCode ReviewerとSecurity Reviewerを別stepとして直列化する。各stepのinputは直前output（先頭だけ`evaluated_commit`）、PhaseRunの`result_commit`は末尾stepのoutputと一致させる。二つのreviewとagent-runを一つのstepへまとめない。

`evaluated_code_commit`の欠落、`review_target.commit_sha`との不一致、`evaluated_commit`の非子孫関係、step連鎖の欠落・分岐・循環、Runnerが記録した`evaluation_output_commit`の非子孫関係もfail-closedとする。PhaseRunまたはGateRunの参照が一件でも欠落、不一致、解決不能、非一意であれば更新を拒否し、各参照を一意に復元できる場合に限り受理する。次の最小schemaで永続化する。
...
次のPhaseRunとGateRunは`last_completed_phase_run_ref`が指すPHASE-7の完了証跡であり、PHASE-8 ready runの子ではない。`evaluation_input_commit`はEvaluator開始前、`result_commit`は信頼済みRunnerがEvaluator成果物だけを追加して固定した出力checkpointである。
...
phase_run:
  ...
  output_revision: 42
  evaluation_input_commit: abc123def456
  evaluation_steps:
    - agent: implementation-evaluator
      input_commit: abc123def456
      output_commit: bcd234efa567
  result_commit: bcd234efa567
  status: passed
  gate_run_refs:
...
`POST_REFACTOR_GREEN`は新しい正式ゲートではなく、既存の`UNIT_TEST_GREEN` GateRunにおけるREFACTOR後の完了段階である。`evaluated_commit`とGateRun内の`result_commit`は参照元PHASE-7の`evaluation_input_commit`に一致させる。`test_evidence_refs`が指す証跡も同じcommit上で生成し、実行した`command`、`exit_code`、`result_summary`、改変検知用の64桁SHA-256 `test_artifact_hash`を束縛する。
...
- gate_run_id: gate-run-TASK-004-implementation-evaluation-007
  ...
  evaluated_commit: abc123def456
  evaluated_code_commit: 890xyz111222
  evaluation_step_input_commit: abc123def456
  evaluation_output_commit: bcd234efa567
  # review_target.commit_shaと一致。実際にコードを読み、テストを実行したcommit。
  # evaluated_code_commitからevaluated_commitまでの差分は
  # review targetとchanges/<task>.yamlだけでなければならない
  status: passed
  review_result_ref: docs/features/order/reviews/TASK-004-implementation.md
  # evaluation_output_commitはEvaluator成果物へ自己記載せず、commit作成後に
  # 信頼済みRunnerがこのGateRunとPhaseRun.result_commitへ記録する
```
...
| REQUIREMENTS_DRAFT | §5.1.2の必須節が揃い、要件ID、受入条件、未解決事項、スコープが明確 | 要件定義 |
...
| Cross-cutting gate | `ACCESS_POLICY`は各AgentRun開始時、`STATE_REVISION`は各`progress.yaml`更新時に反復評価 | `ACCESS_POLICY`, `STATE_REVISION` |
...
| DEC-015 | 要件書の必須節を固定し、成功指標をゲート判定から除外する | 節の欠落を機械判定可能にし、実装完了時点で測定できない指標をゲート条件に持ち込まないため |
...
review:
  ...
  supported_modes:
    ...
    manual: poc_only
  document_consistency: failed
  known_document_consistency_failures:
    - progress.yaml例にPHASE-1、PHASE-3、PHASE-7、PHASE-8のPhase IDと同名のgate keyがない
    - 文書headerはVersion 1.10だが、末尾のappendix / target version抽出結果は1.8である
...
Version 1.12は、**要件書に何を書くかが本書のどこにも定義されていない**という欠落を修正する。§5.1は要件ID、受入条件、前提・制約・スコープ外・未解決事項を列挙していたが、これは「要件書に含める項目の一部」であって文書構成の定義ではなかった。結果として、目的（何のための機能か）と用語定義（ドメイン語彙）を欠いた要件書が`REQUIREMENTS_DRAFT`をPASSし得た。目的が無ければ§5.1「設計・実装上の手段を早期に固定しすぎない」の違反を判定する基準が存在せず、用語定義が無ければ要件・設計・テストの間で語彙が揺れ、§12のトレーサビリティが名寄せ不能になる。

## O.1 変更点

- **§5.1.2 要件書の構成を新設。** 9つの節（目的、用語定義、前提、制約、スコープ外、成功指標、機能要件、非機能要件、未解決事項）を必須として定義し、各節が欠落したときに何が起きるかを併記した。従来未定義だったのは目的、用語定義、成功指標の3節である。
- **成功指標と受入条件の分離を規定。** 判定単位（機能全体 / 要件ごと）、判定時点（リリース後の計測 / 実装完了時点）、判定手段（運用計測 / UT・IT）が異なる。**成功指標を品質ゲートおよび§15 Definition of Doneの判定に使わない**ことを明記した。実装完了時点で測定できない指標をゲート条件へ持ち込むと、必ず判定不能になる。
- **`TBD`の扱いを規定。** 必須節が存在しても内容が`TBD`のみの場合は未充足とする（fail-closed）。確定できない事項は未解決事項へ質問として記録する（§2 推測禁止）。
- **§11のゲート表`REQUIREMENTS_DRAFT`条件を更新。** 必須節の充足を条件へ加えた。節の存在は§11.1の「テンプレート必須欄」として機械判定側で扱う。
- **§5.1冒頭へ要件書の位置付けを追加。** 要件書は人間の合意記録であると同時に、後続する全AgentRunの権威ある入力である。実装がAgentによって高速化されるほど律速は要件記述へ移るため、本工程を省略の対象としない。
- **DEC-015を追加。**
- **`scripts/validate-doc-consistency.sh`の3件の誤検知を修正。** いずれも本書側の欠落ではなく検証スクリプト側の追随漏れであり、付録J.2が`known_document_consistency_failures`として記録していた事象を含む。
  - 品質ゲート表の抽出範囲が`## 11.1`までとなっており、Version 1.10で新設した§11.0の評価順序表（第1列がPhase ID）まで読み込んで`PHASE-1`等をゲート名として扱っていた。終端を`## 11.`へ変更した。
  - 付録の版番号を出現順の末尾から取得していた。付録は新しい版を先頭へ置くため、末尾は常に最古の版になる。版番号として比較した最大値を採るよう変更した。
  - Decision IDの件数を13件で固定していた。連番の検査で欠落は検出できるため、件数の固定を廃し、1件も無い場合だけFAILとした。
- 上記により、本書は`validate-doc-consistency.sh`をFAIL 0件で通過する。

## O.2 影響

- 雛形3件（`workflows/01-requirements.md`、`agents/requirements-analyst.md`、`agents/requirements-reviewer.md`）と`agents/requirements-planner.md`の`deliverables.must_contain`を更新した。
- 既存の要件書は目的・用語定義・成功指標の3節を欠く。次回のPHASE-1差し戻しまたは改訂時に追記すればよく、遡及的な再レビューは求めない。
- ゲートID、状態schema、`progress.yaml`の構造への変更はない。`REQUIREMENTS_DRAFT`の判定条件のみが厳格化される。
...
- **§10.1へEvaluatorの三境界を追加。** `evaluated_code_commit`、Phase共通の`evaluated_commit`、Evaluatorごとのstep input / outputを分離した。入力差分はreview targetと`changes/<task>.yaml`、各stepの出力差分は当該Evaluatorのreview結果とagent-runだけへ限定する。複数Evaluatorはstepを直列化する。Evaluator成果物は自身を含むcommit SHAを自己記載せず、出力commitは作成後にコード系列外のcontrol-state storeへ記録する。
...
- 三境界はreview targetを伴うPHASE-7の`IMPLEMENTATION_EVALUATION`とPHASE-9の`CODE_REVIEW`に適用する。review targetを伴わないGateRunは従来どおり`evaluated_commit`のみとする。
