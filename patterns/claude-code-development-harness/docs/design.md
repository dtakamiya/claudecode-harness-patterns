...
| `PHASE-9` 2軸レビュー | fixed-review-target, test-evidence | code-review, security-review | `INTEGRATION_TEST` | `CODE_REVIEW`, `SECURITY_REVIEW` | code-reviewer, security-reviewer |
| `PHASE-10` 完了監査 | all-artifacts, traceability, reviews | completion-audit, final-handoff | `CODE_REVIEW`, `SECURITY_REVIEW` | `COMPLETION` | completion-auditor |
...
`commit_sha`または同等の不変なレビュー対象が存在しない場合、`IMPLEMENTATION_EVALUATION`、`CODE_REVIEW`、`SECURITY_REVIEW`ゲートを開始してはならない。
...
| 9 | 2軸レビュー | Code Reviewer / Security Reviewer | 正確性レビュー、セキュリティレビュー | 両レビューのblocking指摘ゼロ |
...
| SECURITY_REVIEW | blocking指摘ゼロ、攻撃条件と権限・秘密情報への影響を評価済み | 実装 |
...
- Code ReviewerとSecurity Reviewerのblocking指摘がともにゼロである。

- 要件IDからタスク、UT、IT、実装への追跡が成立している。

- statusとhandoffが最新である。

- レビュー対象のcommit SHA、diff base、変更ファイル一覧が固定され、Implementation Evaluator、Code Reviewer、Security Reviewerが同一対象を検証している。
...
1. git状態を確認し、main/masterでは書込みを止める。既存作業を保護し、ユーザー承認後にfeatureブランチを作成して開始時SHAを記録する。
...
4. `project-initializer`を作成し、production code変更前にリポジトリ構造、ビルド、UT、IT、静的解析を実行してコマンド、終了コード、開始時SHAを`baseline.yaml`へ記録する。
