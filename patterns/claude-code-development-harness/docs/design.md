...
| 対象工程 | 要件定義〜実装完了                                 |
| 版       | Version 1.6 / 2026-07-15（文書整合性・レビューゲート統合版） |
...
| `PHASE-9` コード・セキュリティレビュー | code-review-target, test-evidence, ui-evidence-or-na | code-review, security-review | `CODE_REVIEW_TARGET` | `CODE_REVIEW` | code-reviewer, security-reviewer, context-builder |
...
| `ui_verifier` | Read, Search, Browser / Preview | 固定review targetをread、`docs/tests/ui-evidence/**`とagent-runのみwrite、接続先はローカルpreviewのみ | 表示・操作・viewport・console結果を同一commit SHAの証跡として記録 |
...
| UI Verifier | `docs/tests/ui-evidence/**`, agent-run成果物 | Browser / Previewのみ、ローカルpreview限定 | 専用tool＋接続先allowlist＋write scope | ソースコード修正、外部サイト・本番接続、フォーム送信等の外部更新 |
...
PHASE-7では`kind: implementation_review`、PHASE-8完了後には`kind: code_review`として別々のtargetを作成する。後続工程でファイルまたは証跡が変わった場合、既存targetとレビュー結果をstale化し、新しいcommit SHA、diff base、変更一覧、成果物ハッシュで再固定する。
...
| 9 | コード・セキュリティレビュー | Code Reviewer + Security Reviewer | コードレビュー、セキュリティレビュー | 両レビューのブロッキング指摘ゼロ |
...
`UI_VERIFICATION`の実行者は専用`ui-verifier`とする。`ui-verifier`は固定review targetをread-onlyで受け取り、ローカルpreviewへのBrowser / Preview操作とUI証跡の書込みだけを許可する。Orchestratorと独立Reviewerは判定と証跡を再確認するが、自らUI証跡を生成しない。
...
| Security Reviewer | 入力検証、認証・認可、秘密情報、injection、依存・権限拡大を検査 | Code Reviewerの評価や実装者の説明だけで承認しない |
...
- gate_run_id: gate-run-TASK-004-implementation-review-target-007
  ...
  evaluated_commit: abc123def456
  status: passed
  review_target_ref: docs/reviews/targets/TASK-004-implementation.yaml
- gate_run_id: gate-run-TASK-004-implementation-evaluation-007
  ...
  evaluated_commit: abc123def456
  status: passed
  review_result_ref: docs/reviews/TASK-004-implementation.md
```
...
| CODE_REVIEW | Code ReviewerとSecurity Reviewerのblocking指摘ゼロ、回帰・脆弱性リスク評価済み | 実装 |
...
1. `initializer`を作成し、リポジトリ構造、ビルド、UT、IT、静的解析のベースラインを記録する。
