...
main/master上では書き換えない。既存作業を保護し、ブランチ名候補を提示してユーザー承認後にfeatureブランチを作成する。production code変更前に既存の関連testと該当するtypecheck/lint/buildを実行し、開始時Git SHA、コマンド、終了コード、結果要約をbaselineとして保存する。
