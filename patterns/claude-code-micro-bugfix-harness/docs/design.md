...
| Fix | 根本原因への最小差分を実装 | 回帰テストが成功（GREEN） |
| Verify | 関連test、typecheck/lint/build、必要ならUI preview | 必須コマンドが終了コード0 |
| 2軸Review | 正確性とセキュリティを独立評価 | blocking指摘が0件 |
| Report | 原因、差分、証跡、レビュー、残課題を報告 | 必須項目が存在し、金融分類なし、または固定対象と一致するDevelopment `CODE_REVIEW`証跡あり |
...
- `git push`、PR作成、migration、依存追加は明示承認を要する。
...
Reviewerはread-onlyとし、修正はImplementerへ返す。blocking修正後は回帰テストと関連検証を再実行する。
...
- 顧客資産・取引・会計・規制遵守・金融意思決定に影響する金融ドメインロジック（残高、元帳、注文、約定、決済、送金、返金、金利、手数料、FX、監査、取引状態など）に触れる
...
金融ドメインロジックは局所修正でもDevelopment Harnessへ即時昇格し、Code/Securityの2軸レビューだけで完了してはならない。独立したAdversarial Reviewerを必須とする。金融処理に触れない軽微変更だけはDevelopment HarnessのAdversarial Reviewerが承認した除外判定で除外できる。Micro側で自己除外してはならず、理由と承認をレビュー結果へ記録する。

Triageまたは以降の工程で金融分類となった時点で判定ラベルを`financial_escalation_required`とし、進行中のローカル作業を直ちに停止して、未開始の後続工程へ進まずDevelopment Harnessへ即時昇格する。このラベルは状態機械を追加するものではない。固定対象と一致するDevelopmentの`CODE_REVIEW`証跡（Adversarial Reviewのblocking指摘ゼロ）が記録されるまでReportをブロックする。非金融かつ軽微の場合だけ、DevelopmentのAdversarial Reviewerが承認した除外証跡により解除できる。
