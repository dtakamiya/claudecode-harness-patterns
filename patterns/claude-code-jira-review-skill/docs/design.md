...
（レビュー反映で免責文・想定利用者節・判定値対応表・ブロッカー基準を追加し、その後
reference と重複する Step 3 / Step 5 の補足文を圧縮し、ブロッカー基準は文言を持たない
索引に置き換えた（正典は各 reference の太字。§4.4）結果、SKILL.md は約 210 行。
150 行には未達だが、残りは免責文・想定利用者節・判定値対応表など心理的安全性・
判定一貫性に不可欠な固有記述である。§8 に記載。）
...
**ブロッカー条件の正典は各 reference の該当項目（太字「→ ブロッカー」）に置く。**
SKILL.md「ブロッカーの基準」はその索引であり、条件文言は持たない（二重管理を避ける
ための設計。§8 も参照）。SKILL.md の索引が指す先は次のとおり。

| 群 | 正典の場所 |
|---|---|
| インシデント（記述） | `incident-checklist.md` A（必須フィールド 4 点の欠落）、C（C1 到達点未達 / C2 × / C7 唯一の深い原因が未対応） |
| バグ（記述） | `bug-checklist.md` A2（再現手順なし / 再現不能） |
| QA/QC（種別共通） | `qa-process.md` §0 判定の指針、§1（顧客報告検知でアラートなし）、§2（すり抜け工程未特定）、§3（周知止まり）、§4（回帰テストなし＝インシデント）、§5（過去の同一障害の未検証） |
| アクションアイテム | `action-items.md` 出力表（Owned / Due が ×）、個数の目安（ゼロ件）、カバレッジ（最深 factor が未対応） |
| 種別共通 | `blameless-language.md` 報告の書き方末尾（非難的表現が根本原因欄・アクションアイテム欄にある） |

条件を追加・変更するときは reference 側だけを直す。SKILL.md / 本節は「どのファイルの
どの項目か」がずれたときだけ更新する。
...
実使用で誤検出・見逃しが出たら 1 件ずつ足す。**まず症状を 3 分類する**（詳細な
判断基準は `reference/false-positives.md`「おかしい指摘を見たときのトリアージ」）。

1. **判定基準の不足**（見逃し）なら、該当する `reference/*.md` に判定行を 1 行追記。
   ブロッカー条件なら太字「→ ブロッカー」で書く（SKILL.md の索引が拾う。§4.4）
2. **偽陽性・過剰指摘** なら、`reference/false-positives.md` に FP を 1 件追記
   （Step 6 で照合される。diff-review の `gotchas.md` に相当）
3. **スコープ外**（1 枚のチケットから判定できないこと）なら、同ファイルに FP を追記し
   「要確認」に留めるべき旨を書く
4. いずれも `evals/tasks.jsonl` に対応タスク（見逃し→負例 / 過剰指摘→正例）を 1 件追加
...
- **SKILL.md が 150 行目標を超過**（約 210 行）。reference と重複する Step 3 / Step 5 /
  ブロッカー基準の補足文は圧縮済み（ブロッカー基準は文言を持たない索引に置き換え、
  条件の正典は各 reference の太字に一本化した。§4.4）。150 行は未達だが、残りは免責文・
  想定利用者節・判定値対応表など心理的安全性・判定一貫性に不可欠な固有記述。
